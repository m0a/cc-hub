...
## デフォルトのフロー — Codex 側で完結 (推奨)

同梱の `scripts/profile.ts` が CDP の WebSocket を直接叩いて `.cpuprofile` を取る。
**Chrome / DevTools を立ち上げる必要はなく、Codex セッション内で完結する。**

```bash
# CC Hub プロジェクト内から:
bun .Codex/skills/cchub-profile/scripts/profile.ts profile --seconds 30 --out /tmp/p.json
```
...
bun .Codex/skills/cchub-profile/scripts/profile.ts analyze /tmp/p.json

# 特定関数を含むスタックの leaf 集計
bun .Codex/skills/cchub-profile/scripts/profile.ts drill /tmp/p.json buildSessionsList
```
...
> **agent-browser 経由は不可**: HTTPS の debug.bun.sh から `ws://` の mixed-content / WebSocket 接続まわりの制約で Timeline のイベント列が空になる。Codex 側からの取得は必ず上記の `scripts/profile.ts` ルートを使う。
