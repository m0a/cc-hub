...
export const InputBar = memo(function InputBar({
  ...
  const resetPositionToggleTimer = useCallback(() => {
    if (positionToggleTimeoutRef.current) clearTimeout(positionToggleTimeoutRef.current);
    positionToggleTimeoutRef.current = window.setTimeout(() => setShowPositionToggle(false), 3000);
  }, []);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await authFetch(`${API_BASE}/api/upload/image`, { method: 'POST', body: formData });
  ...
  const handleInputBarTouchStart = (e: React.TouchEvent) => {
    inputBarSwipeRef.current = { startX: e.touches[0].clientX, startY: e.touches[0].clientY };
  };
  ...
  const handleInputBarTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(deltaX) > 50 && Math.abs(deltaX) > Math.abs(deltaY)) {
      ...
      if (deltaX > 0 && inputMode === 'shortcuts') {
        setIsAnimating(true);
        setInputMode('input');
        setTimeout(() => { setIsAnimating(false); inputRef.current?.focus(); fitTerminal(); }, 350);
      } else if (deltaX < 0 && inputMode === 'input') {
        setIsAnimating(true);
        setInputMode('shortcuts');
        setInputValue('');
        setTimeout(() => { setIsAnimating(false); fitTerminal(); }, 350);
      }
    }
  ...
  return (
    ...
    >
      ...
      {!showOverlay && overlayContent && onOverlayTap && (
        <div className="h-4 flex items-center justify-center" onClick={onOverlayTap}>
      ...
      {!overlayContent && (
        <div
          className={`bg-th-surface flex justify-between items-center overflow-hidden transition-all duration-300 ${
            isTablet ? (showPositionToggle ? 'px-2 py-1' : 'h-0 py-0') : 'px-2 py-1'
          }`}
          onClick={() => isTablet && !showPositionToggle && setShowPositionToggle(true)}
        >
          {(!isTablet || showPositionToggle) && (
            <span className="text-xs text-th-text-muted">{showHint && 'スクロールで閉じる'}</span>
          )}
          {isTablet && inputMode === 'shortcuts' && showPositionToggle && (
            <button onClick={handlePositionToggle} className="px-2 py-0.5 bg-th-surface-hover text-th-text-secondary text-xs rounded">
      ...
      {isTablet && !showPositionToggle && inputMode === 'shortcuts' && (
        <div className="h-2 bg-th-surface flex items-center justify-center" onClick={() => setShowPositionToggle(true)}>
              ...
              {showInputHistory && inputHistoryRef.current.length > 0 && (
                <div className="max-h-40 overflow-y-auto border border-th-border rounded bg-th-bg mb-1">
                  {inputHistoryRef.current.map((item, i) => (
                    <button
                      key={`${i}-${item}`}
                      onClick={() => { setInputValue(item); setShowInputHistory(false); inputRef.current?.focus(); }}
        ...
        <div className="bg-[#111111]">
          <div className="flex items-center justify-between px-2 py-1.5 border-b border-white/[0.04]">
            <div className="inline-flex bg-white/[0.04] rounded-md p-0.5">
              ...
              <button className="px-3 py-1 text-[11px] bg-white/[0.08] text-zinc-300 rounded font-medium">
                入力
              </button>
            </div>
            <button
              onClick={() => { setInputMode('hidden'); fitTerminal(); }}
