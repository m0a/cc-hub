...
### HistoryEntry（履歴検索応答 — 実機確認済み / M1 確定）
履歴検索（`/history/search(/stream)`）の各結果。実フィールド（`tui/src/api/history.ts`）:
- `sessionId: string` — `POST /history/resume` の再開キー
- `projectPath: string` — プロジェクト（作業ディレクトリ、resume にも使用）
- `projectName?: string`
- `firstPrompt?: string` / `lastPrompt?: string` — 表示用の手がかり
- `gitBranch?: string`
- `modified?: string`（ISO 日時）
- `agent?: AgentProvider`
